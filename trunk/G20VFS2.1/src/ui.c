...
int identify_command(char input[])
{
	...
	else if(strcmp(ch,"deletedir")==0)
	{
		process_command(14,input);	
	}
	else if(strcmp(ch,"movedir")==0)
	{
		process_command(15,input);	
	}
	else if(strcmp(ch,"copydir")==0)
	{
		process_command(16,input);	
	}
...
int process_command(int i,char input[]){
	...
	switch(i)
	{
		...
		case 14 :
			if( validate_delete_dir(input)==1)
			{
...
int man()
{
	...
	puts("\n=============================================");
	puts("man - for help.");
	puts("create_vfs <vfs name> <size in KB> : To create New VFS.");
