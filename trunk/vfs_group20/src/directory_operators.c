...
//        puts("makedir_FAILURE <DIRECTORY_ALREADY_EXISTS>");
    ...
    //printf("\nGot ii parent=%s",parent->file_desp.file_name);
...
//	Create & Store FD 
    ...
    //   puts("display:");
      //   display_nary_c(NARY_ROOT);    
		...
		//printf("\nName=%s path=%s",file_desp.file_name,file_desp.file_location_full_path);
...
//        puts("deletedir_FAILURE <INVALID_DIRNAME>");
    ...
    //check If dir empty or not
     ...
     //printf("\nparent Path %s",parent_nary->file_desp.file_location_full_path);
     ...
     if(parent_nary->child == bstnode->nary_node)
     {
        ...
        //printf("\nnew chlid Path %s",parent_nary->child->file_desp.file_location_full_path);
        //Update FD
...
//        puts("movedir_FAILURE <DESTINATION_CANNOT_BE_FILE>");
   	  printf("\nmovedir_FAILURE %s",ERR_VFS_MOVEDIR_07);
...
*/
     ...
     ///// uPATES  BST & hASHTABLE ////
        ...
        */
        ...
        //printf("\n\n\nin hash name=%s path%s",node->file_desp.file_name,new_path);
...
int list_dir(char dir_path[],int flag, FILE *fout )
{
    ...
    if(bstnode==NULL)
    {
        //puts("listdir_FAILURE <CANNOT_FIND_SPECIFIED_PATH_OR_DIR>");
   	  printf("\nlistdir_FAILURE %s",ERR_VFS_LISTDIR_01);
